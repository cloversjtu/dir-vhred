...
class VariationalSolver(Solver):
    ...
    def train(self):
        ...
        for epoch_i in range(self.epoch_i, self.config.n_epoch):
            ...
            for batch_i, (conversations, conversation_length, sentence_length) \
                ...
                # batch_loss = recon_loss +( 1/recon_loss) * kl_div 
                if self.config.kl_threshold != 0:
                    batch_loss = recon_loss + torch.gt(kl_div, (self.config.kl_threshold*n_words.item())) * kl_div
                else:
                    # batch_loss = recon_loss + kl_mult * kl_div
                    batch_loss = recon_loss + 1.0/(recon_loss) * kl_div
                batch_loss_history.append(batch_loss.item())
            ...
            torch.cuda.empty_cache()   # empty the cashe
    ...
    def generate_sentence(self, sentences, sentence_length,
        ...
        if self.config.mode == 'test':
            file_path = 'test_samples.txt'
        with open(os.path.join(self.config.save_path, file_path), 'a') as f:
            f.write(f'<Epoch {self.epoch_i}>\n\n')
    ...
    def importance_sample(self):
        ...
        for batch_i, (conversations, conversation_length, sentence_length) \
            ...
            for j in range(self.config.importance_sample):
                ...
                log_w = (-recon_loss.sum() - kl_div).data
                #log_w = (-recon_loss.sum() + 1.0/(recon_loss.sum())*kl_div).data
                ...
                if j == 0:
                    n_total_words += n_words.item()
                    kl_div_history.append(kl_div.item())
                    rl_history.append(recon_loss.item())
                    loss_history.append(log_w.item())

            # weights: [n_samples]
            weights = torch.stack(weights, 0).cpu()
            # print(weights)
            # sys.exit(1)
        ...
        epoch_kl_div = np.sum(kl_div_history) / n_total_words
        epoch_rl = np.sum(rl_history) / n_total_words
        epoch_loss = np.sum(loss_history) / n_total_words

        print_str = f'Word perplexity upperbound using {self.config.importance_sample} importance samples: {word_perplexity:.3f}, kl_div: {epoch_kl_div:.3f}, rl: {epoch_rl:.3f}, loss: {epoch_loss:.3f}\n'
        print(print_str)
    ...
    def generate_file(self):
        ...
        for batch_i, (conversations, conversation_length, sentence_length) \
            ...
            batch_loss = recon_loss + kl_div
            if self.config.bow:
                bow_loss = self.model_eval.compute_bow_loss(target_conversations)
                bow_loss_history.append(bow_loss.item())
    ...
    def gen_one_sentence(self, sentence, length):
        word2vec = getattr(self, 'word2vec', None)
        if word2vec is None:
            print('Loading word2vec model')
            word2vec = gensim.models.KeyedVectors.load_word2vec_format(word2vec_path, binary=True)
        ...
        with torch.no_grad():
            context = to_var(torch.LongTensor(sentence))
            sentence_length = to_var(torch.LongTensor(length))
        samples = self.model.generate(context, sentence_length, n_context)
