...
data_dict = {'cornell': data_dir.joinpath('cornell'), 'ubuntu': data_dir.joinpath('ubuntu'), 'dailydialog': data_dir.joinpath('dailydialog')}
optimizer_dict = {'RMSprop': optim.RMSprop, 'Adam': optim.Adam}
rnn_dict = {'lstm': nn.LSTM, 'gru': nn.GRU}
rnncell_dict = {'lstm': StackedLSTMCell, 'gru': StackedGRUCell}
username = Path.home().name
save_dir = Path(f'/home/{username}/notespace/data3/conversation/')
...
class Config(object):
    def __init__(self, **kwargs):
        ...
        if self.mode == 'train' and self.checkpoint is None:
            ...
            # self.save_path = save_dir.joinpath(self.data, self.model, "epoch_manual")
            self.kl_log_dir = self.save_path.joinpath('validation_log.txt')
            ...
            self.kl_log_dir = os.path.join(self.save_path,'validation_log.txt')
            self.logdir = self.save_path
        elif self.mode != 'train' and self.eval_checkpoint is not None:
            assert os.path.exists(self.eval_checkpoint)
            self.save_path = self.eval_checkpoint
            self.kl_log_dir = os.path.join(self.save_path,'validation_log.txt')
            self.logdir = self.save_path
...
def get_config(parse=True, **optional_kwargs):
    ...
    # Model
    parser.add_argument('--model', type=str, default='DIR_VHRED',
                        help='DIR_VHRED')
    ...
    # One Sentence mode
    parser.add_argument('--one_sentence', type=str, default=None)
    parser.add_argument('--one_latent_z', type=int, default=None)
